...
plotmatch_VerCrossHori = function(filtered_stitched_polygon_list, filtered_stitched_polygon_list_name, plot_dt ){
  ...
  x_mean_list = x_mean_list[order(x_mean_list)]
  
  output = get_seq(rcoords$Y, row_n)
  y_seq = output[[1]]
  y_mean_list = output[[2]]
  y_mean_list = y_mean_list[order(y_mean_list)]
  ...
  for(m in 1:row_n){
    for(n in 1:col_n){
      ...
      if(paste(n, m, sep="_") %in% x_y_tag){
        ...
        ### add coordinates
        sub_dt = rcoords[which(y_seq == m), ]
        model <- lm(Y ~ X, data=sub_dt)
        y_pre = predict(model, data.frame(X = x_mean_list[n]))
        
        sub_dt = rcoords[which(x_seq == n), ]
        model <- lm(X ~ Y, data=sub_dt)
        x_pre = predict(model, data.frame(Y = y_mean_list[m]))
